...
pub fn get_secret_key(private_key_string: String, secp: &Secp256k1) -> Option<SecretKey> {
    ...
    if length == 52 {
        // Compressed WIF
        let private_key_check: Vec<u8> =
            FromBase58::from_base58check(&private_key_string).unwrap();
        ...
        let private_key_hex: String = String::from(
            &private_key_hex_check[2..private_key_hex_check.len() - 2],
        );
        ...
        println!("Unrecognized private key format for {}. Use Compressed WIF or HEX format", private_key_string);
        None
    }
}
